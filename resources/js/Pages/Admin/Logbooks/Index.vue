    ...
    >
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-blue-100 text-sm font-medium">Total Logbook</p>
                        <p class="text-3xl font-bold">{{ stats.total_logbooks || 0 }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-yellow-100 text-sm font-medium">Perlu Review</p>
                        <p class="text-3xl font-bold">{{ stats.pending_logbooks || 0 }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ stats.approved_logbooks || 0 }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-purple-100 text-sm font-medium">Rata-rata Jam/Hari</p>
                        <p class="text-3xl font-bold">{{ stats.average_hours || 0 }}</p>
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 mt-1">
                            Menampilkan {{ filteredLogbooks.length }} dari {{ logbooks.length }} logbook
                        </p>
                    </div>
                        ...
                        >
                                ...
                                <div class="flex items-center">
                                    <div class="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                                        <span class="text-sm font-semibold text-blue-600">
                                            {{ logbook.user.name.charAt(0).toUpperCase() }}
                                        </span>
                                    </div>
                                    <div class="ml-4">
                                        ...
                                        <div v-if="logbook.user.division" class="text-xs text-gray-400">
                            ...
                            <td class="px-6 py-4">
                                <div class="max-w-xs">
                                    ...
                                    <div v-if="logbook.attachments && logbook.attachments.length > 0" class="flex items-center mt-1">
                                        <svg class="w-3 h-3 mr-1 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                                        </svg>
                                        <span class="text-xs text-gray-400">{{ logbook.attachments.length }} file</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-sm text-gray-900">{{ logbook.duration || 0 }} jam</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <Link
                                        :href="route('admin.logbooks.show', logbook.id)"
            ...
            <div v-else class="p-12 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 mb-2">Tidak ada logbook ditemukan</h3>
                <p class="text-gray-500">Coba ubah kriteria pencarian atau filter yang Anda gunakan</p>
            </div>
        </div>
    </AdminLayout>
</template>
...
const filteredLogbooks = computed(() => {
    return props.logbooks.filter(logbook => {
        const matchesSearch = !searchQuery.value || 
            logbook.user.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
            logbook.title.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
            (logbook.activities && logbook.activities.toLowerCase().includes(searchQuery.value.toLowerCase()));
        
        const matchesStatus = !statusFilter.value || logbook.status === statusFilter.value;
        
        const matchesDivision = !divisionFilter.value || 
            (logbook.user.division && logbook.user.division.id === parseInt(divisionFilter.value));
        
        const matchesMonth = !monthFilter.value || 
            (logbook.date && logbook.date.startsWith(monthFilter.value));
        
        return matchesSearch && matchesStatus && matchesDivision && matchesMonth;
    });
});

const isAllSelected = computed(() => {
    return filteredLogbooks.value.length > 0 && 
...
const toggleSelectAll = () => {
    if (isAllSelected.value) {
        ...
        selectedLogbooks.value = filteredLogbooks.value.map(logbook => logbook.id);
    }
};
...
const quickApprove = (logbookId) => {
    router.put(route('admin.logbooks.approve', logbookId), {}, {
        preserveScroll: true,
        onSuccess: () => {
            // Remove from selected if it was selected
            selectedLogbooks.value = selectedLogbooks.value.filter(id => id !== logbookId);
        }
    });
};
...
const bulkAction = (action) => {
    ...
    router.post(route('admin.logbooks.bulk-update'), {
