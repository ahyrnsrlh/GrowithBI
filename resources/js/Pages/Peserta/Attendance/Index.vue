    ...
    <div class="bg-[#2F4C9E] text-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <nav class="text-sm mb-4 flex items-center space-x-2 text-blue-100">
                ...
                <Link :href="route('profile.edit')" class="hover:text-white transition-colors cursor-pointer">
                    Dashboard
                </Link>
                <span>›</span>
            ...
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-blue-100 mt-1">Kelola kehadiran Anda dengan mudah</p>
                </div>
            </div>

            <!-- Real-time Clock -->
            <div class="mt-4 inline-flex items-center space-x-2 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-lg">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <transition
                ...
                >
                    ...
                    >
                        <div class="bg-white rounded-2xl shadow-lg border border-green-200 p-4 flex items-start space-x-3">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="mt-1 text-sm text-gray-600">{{ $page.props.flash.success }}</p>
                            </div>
                            <button @click="showSuccessToast = false" class="flex-shrink-0 text-gray-400 hover:text-gray-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                >
                    <div
                        ...
                        class="fixed top-4 right-4 z-50 max-w-md"
                    >
                        <div class="bg-white rounded-2xl shadow-lg border border-red-200 p-4 flex items-start space-x-3">
                            <div class="flex-shrink-0">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="mt-1 text-sm text-gray-600">{{ $page.props.flash.error }}</p>
                            </div>
                            <button @click="showErrorToast = false" class="flex-shrink-0 text-gray-400 hover:text-gray-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                </svg>
                            </button>
                ...
                <div class="bg-white rounded-2xl shadow-md overflow-hidden mb-8">
                    ...
                    <div class="p-6">
                        <div class="grid md:grid-cols-2 gap-6 mb-8">
                            <!-- Check-in Status -->
                            <div class="relative bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border-2 border-green-200 transition-all duration-300 hover:shadow-lg">
                                <div class="flex items-start justify-between">
                                    ...
                                    <div class="flex-shrink-0">
                                        <div v-if="todayAttendance?.check_in" class="w-12 h-12 bg-green-600 rounded-full flex items-center justify-center">
                            ...
                            <!-- Check-out Status -->
                            <div class="relative bg-gradient-to-br from-[#E8EEF7] to-[#D6E3F5] rounded-xl p-6 border-2 border-[#B4C7E7] transition-all duration-300 hover:shadow-lg">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <div class="flex items-center space-x-2 mb-2">
                                            <svg class="w-5 h-5 text-[#2F4C9E]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                                            </svg>
                                            <span class="text-sm font-medium text-[#2F4C9E]">Check-out</span>
                                        </div>
                                        <div
                                            class="text-3xl font-bold transition-colors duration-300"
                                            :class="
                                    ...
                                    <div class="flex-shrink-0">
                                        <div v-if="todayAttendance?.check_out" class="w-12 h-12 bg-[#2F4C9E] rounded-full flex items-center justify-center">
                                            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                            </svg>
                                        </div>
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <button
                                    ...
                                    "
                                >
                                    <svg v-if="isProcessing && actionType === 'check-in'" class="animate-spin h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24">
                                ...
                                <div v-if="!todayAttendance?.check_in && !isWithinCheckInTime" class="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded-lg">
                            ...
                            <div>
                                <button
                                    ...
                                    "
                                >
                                    <svg v-if="isProcessing && actionType === 'check-out'" class="animate-spin h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <svg v-else class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div v-if="todayAttendance?.check_in && !todayAttendance?.check_out && !isWithinCheckOutTime" class="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded-lg">
                                    <p class="text-xs text-yellow-800 text-center">
                ...
                <div class="bg-white rounded-2xl shadow-md overflow-hidden">
                    <div class="bg-[#2F4C9E] px-6 py-4 border-b border-[#274089]">
                        ...
                        <!-- Summary Stats -->
                        <div v-if="monthlyStats" class="mt-4 flex items-center space-x-6 text-sm">
                            <div class="flex items-center space-x-2">
                                <div class="w-3 h-3 bg-white rounded-full"></div>
                                <span class="text-white">Total Hadir: <span class="font-bold">{{ monthlyStats.total }}</span> hari</span>
                            </div>
                            <div class="flex items-center space-x-2">
                                <div class="w-3 h-3 bg-green-300 rounded-full"></div>
                                <span class="text-white">Tepat Waktu: <span class="font-bold">{{ monthlyStats.onTime }}</span></span>
                            </div>
                            <div class="flex items-center space-x-2">
                                <div class="w-3 h-3 bg-yellow-300 rounded-full"></div>
                                <span class="text-white">Terlambat: <span class="font-bold">{{ monthlyStats.late }}</span></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                            ...
                            >
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center space-x-3 flex-1">
                                        ...
                                        <div>
                                            <p class="font-semibold text-gray-900">{{ formatDate(att.date) }}</p>
                                            <p class="text-xs text-gray-500">{{ formatDayName(att.date) }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <!-- Check-out -->
                                    <div class="flex items-center space-x-2 flex-1">
                                        <svg class="w-4 h-4 text-[#2F4C9E]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                                        </svg>
                                        <div>
                        ...
                        <div v-else class="text-center py-12">
                            ...
                            <p class="text-gray-400 text-sm mt-2">Lakukan check-in untuk memulai</p>
                        </div>
                        ...
                        >
                            <div class="text-sm text-gray-600">
                                Menampilkan {{ ((currentPage - 1) * perPage) + 1 }} - {{ Math.min(currentPage * perPage, filteredAttendance.length) }} dari {{ filteredAttendance.length }} data
                            </div>
                            <div class="flex items-center space-x-2">
                                <button
                                    @click="currentPage--"
...
const fetchServerTime = async () => {
    try {
        ...
        console.log('Last sync at:', new Date(lastSyncTime.value).toLocaleString());
        console.log('=========================');
...
const updateClockDisplay = () => {
    ...
    const formattedTime = `${String(now.hours).padStart(2, '0')}:${String(now.minutes).padStart(2, '0')}:${String(now.seconds).padStart(2, '0')}`;
...
const isWithinCheckInTime = computed(() => {
    ...
    const [startHour, startMin] = checkInTimeRange.value.check_in.start.split(':').map(Number);
    const [endHour, endMin] = checkInTimeRange.value.check_in.end.split(':').map(Number);
...
const isWithinCheckOutTime = computed(() => {
    ...
    const [startHour, startMin] = checkInTimeRange.value.check_out.start.split(':').map(Number);
    const [endHour, endMin] = checkInTimeRange.value.check_out.end.split(':').map(Number);
