    ...
    <AuthenticatedLayout>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ stats.approved_reports || 0 }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ stats.pending_reports || 0 }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-red-500 to-red-600 rounded-xl shadow-lg p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-red-100 text-sm font-medium">Perlu Revisi</p>
                        <p class="text-3xl font-bold">{{ stats.revision_reports || 0 }}</p>
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-xl font-semibold text-gray-900">Laporan Akhir Saya</h2>
                <p class="text-sm text-gray-600 mt-1">Daftar laporan yang telah Anda upload</p>
            </div>

            <div v-if="reports && reports.length > 0" class="divide-y divide-gray-200">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center space-x-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h3 class="text-sm font-medium text-gray-900 truncate">{{ report.title }}</h3>
                                    <p class="text-sm text-gray-500 truncate">{{ report.file_name }}</p>
                                    <div class="flex items-center space-x-4 mt-1">
                                        <span class="text-xs text-gray-500">{{ formatFileSize(report.file_size) }}</span>
                                        <span class="text-xs text-gray-500">{{ formatDate(report.created_at) }}</span>
                                    </div>
                                </div>
                            </div>
                            <p v-if="report.description" class="mt-2 text-sm text-gray-600">{{ report.description }}</p>
                            <div v-if="report.feedback && report.status === 'revision'" class="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                                <p class="text-sm text-yellow-800"><strong>Feedback:</strong> {{ report.feedback }}</p>
                            </div>
                        </div>
                        <div class="flex items-center space-x-3 ml-4">
                            <span
                                :class="[
                                    ...
                                    report.status === 'revision' ? 'bg-red-100 text-red-800' :
                                    'bg-gray-100 text-gray-800'
                                ]"
                            >
                                {{
                                    report.status === 'approved' ? 'Disetujui' :
                                    report.status === 'submitted' ? 'Menunggu Review' :
                                    report.status === 'revision' ? 'Perlu Revisi' :
                                    'Draft'
                                }}
                            </span>
                            <div class="flex space-x-2">
                                <a
                                    :href="route('profile.reports.download', report.id)"
                                    class="text-blue-600 hover:text-blue-700 text-sm font-medium"
                                    title="Download"
                                >
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-4-4m4 4l4-4m-4-4V2" />
                                    </svg>
                                </a>
                                <Link
            ...
            <div v-else class="p-12 text-center">
                ...
                <p class="text-gray-600 mb-6">Upload laporan akhir magang Anda untuk mendapatkan review dari pembimbing.</p>
                <Link
                    :href="route('profile.reports.create')"
