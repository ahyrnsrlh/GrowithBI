<template>
  <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
    <div class="flex items-center justify-between mb-4">
      ...
      <div v-if="currentFile" class="flex items-center text-green-600 bg-green-100 px-2 py-1 rounded-full">
      ...
      <div class="flex items-start justify-between">
        <div class="flex items-center flex-1 min-w-0">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-green-800 truncate">{{ getFileName(currentFile) }}</p>
            <p class="text-xs text-green-600 mt-1">File berhasil terupload</p>
          </div>
        </div>
    ...
    >
      <div class="flex flex-col items-center">
        ...
        <label class="inline-flex items-center px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg cursor-pointer hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors">
    ...
    <div v-if="uploading" class="mt-6">
      <div class="flex items-center justify-between mb-3">
        <span class="text-sm font-medium text-gray-700">Mengupload file...</span>
    ...
    <div class="mt-4 p-3 bg-gray-50 rounded-lg">
      <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-600">
        <span class="flex items-center"><i class="fas fa-info-circle mr-1.5 text-blue-500"></i>Maksimal 5MB</span>
        <span class="flex items-center"><i class="fas fa-file-alt mr-1.5 text-green-500"></i>PDF, JPG, PNG</span>
        <span v-if="required" class="flex items-center"><i class="fas fa-asterisk mr-1.5 text-red-500"></i>Wajib diisi</span>
      </div>
    </div>
  </div>
</template>
