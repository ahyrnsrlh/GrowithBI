...
Route::middleware('auth')->group(function () {
    ...
    Route::prefix('profile/attendance')->name('profile.attendance.')->group(function () {
        ...
        Route::post('/verify-face', [\App\Http\Controllers\Peserta\AttendanceController::class, 'verifyFace'])->name('verify-face')->middleware('throttle:10,1');
    });
...
Route::prefix('admin')->name('admin.')->middleware(['auth', 'verified', 'role:admin,pembimbing', '2fa'])->group(function () {
    ...
    Route::put('/notifications/{notification}/mark-read', [\App\Http\Controllers\NotificationController::class, 'markAsRead'])->name('notifications.mark-read');
    ...
    // Export capabilities for final reports (must come before {report} param route)
